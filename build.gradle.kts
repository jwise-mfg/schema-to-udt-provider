...
// Check for code signing files before build
val keystoreFile = file("code-signing/keystore.jks")
val certFile = file("code-signing/codesigning.p7b")
val gradlePropsFile = file("gradle.properties")

if (!keystoreFile.exists() || !certFile.exists() || !gradlePropsFile.exists()) {
    val missing = mutableListOf<String>()
    if (!keystoreFile.exists()) missing.add("code-signing/keystore.jks")
    if (!certFile.exists()) missing.add("code-signing/codesigning.p7b")
    if (!gradlePropsFile.exists()) missing.add("gradle.properties")

    throw GradleException("""

        ========================================
        CODE SIGNING FILES NOT FOUND
        ========================================

        Missing: ${missing.joinToString(", ")}

        To generate the required signing files, run:

            ./code-signing/generate-keystore.sh

        This will create the keystore, certificate files, and gradle.properties.
...
ignitionModule {
    ...
    moduleDescription.set("Provide JSON Schema as UDT.")
     ...
     */
    hooks.putAll(mapOf(
        "com.theoremsystems.ignition.schematagprovider.gateway.SchemaTagProviderGatewayHook" to "G"
    ))
